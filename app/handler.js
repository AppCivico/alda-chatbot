		...
		await Sentry.configureScope(async (scope) => {
			...
			try {
				...
				case 'calendar': // agenda
					await Sentry.configureScope(async (scope2) => {
						await context.typingOn();
						await context.setState({ agenda: await db.getAgenda(context.state.CCS.id) });
						scope2.setUser({ username: context.session.user.first_name });
						scope2.setExtra('agenda', context.state.agenda);
				...
				case 'results':
					...
					// If we have an agenda but no results for that agenda we show the results from the most recent agenda.
						...
						scope2.setUser({ username: context.session.user.first_name });
						scope2.setExtra('results', context.state.results);
				...
				case 'findLocation': { // user sends geolocation, we find the bairro using googleMaps and confirm at the end
					...
					// scope.setExtra();
						...
						try {
							...
							scope2.setExtra('mapsResults.status', context.state.mapsResults.status);
							if (context.state.mapsResults.status === 200) {
								...
								if (await help.checkIfInRio(context.state.mapsResults) === true) { // we are in rio
									...
									scope2.setExtra('mapsCity', context.state.mapsCity);
									if (!context.state.mapsCity) {
										...
										if (context.state.mapsBairro) {
											scope2.setExtra('mapsBairro', context.state.mapsBairro);
