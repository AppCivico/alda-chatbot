...
module.exports = async (context) => {
  if (!context.event.isDelivery && !context.event.isEcho) {
    try {
      ...
      await appcivicoApi.postRecipientMA(context.state.politicianData.user_id, {
        ...
        if (!context.state.dialog) { // in case a user manages to send a text message without starting the dialog properly
          ...
          switch (context.state.dialog) {
            ...
            default: // regular text message => error treatment
              ...
              if (await createIssue(context) === true) {
                await context.sendText('Salvei a sua mensagem. Logo mais meus criadores estarão respondendo sua dúvida.\n'
